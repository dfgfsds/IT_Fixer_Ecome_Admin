...
export function AddressForm({ openModal, handleClose, editData, setEditData, pickupValue }: AddressFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-950/50 backdrop-blur-xs flex justify-center items-center p-4 overflow-y-auto">
      <div
        className="bg-white p-6 rounded-2xl shadow-xl max-w-xl w-full border border-gray-100 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center pb-3 mb-4 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">{editData ? 'Edit Address' : 'Add Your Address'}</h2>
          <button type="button" onClick={() => { handleClose(); setEditData(''); reset(); }} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-2 bg-[#e2ba2b] text-white rounded-md text-sm font-medium hover:bg-[#a6d719]-300 disabled:opacity-50 flex gap-2"
            >
